import { findCharacterByIdAroundPosition } from "../../character/character.js";
...
function createAbilityObjectBoomerang(
    ...
    let object: AbilityObjectPetBoomerang = {
        ...
        deleteTime: game.state.time + 5000,
...
function setAbilityPetBoomerangToLevel(ability: Ability, level: number) {
    const abilityPetBoomerang = ability as AbilityPetBoomerang;
}
    ...
    if (!petBoomerang.nextTickTime || petBoomerang.nextTickTime <= game.state.time) {
        ...
        if (!petBoomerang.hitTarget) {
            if (petBoomerang.targetIdRef !== undefined) {
                const target = findCharacterByIdAroundPosition(petBoomerang, 200, game, petBoomerang.targetIdRef);
                if (target && target.state === "alive") {
                    petBoomerang.direction = calculateDirection(petBoomerang, target);
                } else {
                    //set target position
                }
...
function tickAbilityPetBoomerang(abilityOwner: AbilityOwner, ability: Ability, game: Game) {
    ...
    if (!abilityPetBoomerang.lastThrowTime || abilityPetBoomerang.lastThrowTime + abilityPetBoomerang.throwInterval < game.state.time) {
        abilityPetBoomerang.lastThrowTime = game.state.time;
        const boomerangObject = createAbilityObjectBoomerang(abilityOwner, abilityPetBoomerang.baseDamage, abilityPetBoomerang.radius, abilityOwner.faction, abilityPetBoomerang.id, undefined, 0, game);
