...
function getOptionsSplit(ability: Ability): UpgradeOptionAndProbability[] {
    let options = getAbilityUpgradeOptionDefault(ability, ABILITY_PET_PAINTER_UPGARDE_SPLIT);
...
function getAbilityUpgradeSplitUiText(ability: Ability): string {
    let up: AbilityPetPainterUpgradeSplit = ability.upgrades[ABILITY_PET_PAINTER_UPGARDE_SPLIT];
...
function getAbilityUpgradeSplitUiTextLong(ability: Ability): string[] {
    ...
    textLines.push(`into new smaller ones`);
