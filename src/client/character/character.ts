...
export function tickCharacters(characters: (Character | undefined)[], game: Game, pathingCache: PathingCache | null, petOwner?: Character) {
    for (let j = characters.length - 1; j >= 0; j--) {
        ...
        if (!char) continue;
        if ((game.state.bossStuff.kingFightStartedTime !== undefined || game.state.bossStuff.areaSpawnFightStartedTime !== undefined) && char.type === CHARACTER_TYPE_ENEMY_FIX_RESPAWN_POSITION) continue;
...
export function calculateAndSetMoveDirectionToPositionWithPathing(character: Character, targetPosition: Position | null, map: GameMap, pathingCache: PathingCache | null, idCounter: IdCounter, time: number, game: Game): boolean {
    ...
    const nextWayPoint: Position | null = getNextWaypoint(character, targetPosition, pathingCache, time, game);
