...
import { addSpellmakerToolOrbiter, SPELLMAKER_TOOL_ORBITER, SpellmakerCreateObjectMoveAttachmentOrbiter, SpellmakerCreateToolMoveAttachmentOrbiter } from "./spellmakerToolOrbiter.js";
...
export type AbilitySpellmakerObject = AbilityObject & {
    ...
    debuffAttachment?: SpellmakerCreateToolDebuffAttachment,
}
...
export type SpellmakerCreateToolDebuffAttachment = {
    type: string,
}
    ...
    if ((currentStage === displayStage - 1 && toolFunctions.canHaveNextStage) || currentStage === displayStage) {
        if (toolFunctions.paint) {
            ...
            if (createdObject.debuffAttachment) {
                const toolMoveFunctions = SPELLMAKER_DEBUFF_TOOLS_FUNCTIONS[createdObject.debuffAttachment.type];
                toolMoveFunctions.paint(ctx, createdObject.debuffAttachment, ownerPaintPos, ability, game);
            }
        }
    }
...
function paintToolObjectsRecusiveForCharging(ctx: CanvasRenderingContext2D, abilityOwner: AbilityOwner, currentStage: number, createdObject: SpellmakerCreateToolObjectData, ability: AbilitySpellmaker, paintPos: Position, chargeFactor: number, spell: SpellmakerSpell, game: Game) {
    const toolFunctions = SPELLMAKER_TOOLS_FUNCTIONS[createdObject.type];
    ...
    if (toolFunctions.paint) {
        ...
        if (createdObject.debuffAttachment) {
            const toolMoveFunctions = SPELLMAKER_DEBUFF_TOOLS_FUNCTIONS[createdObject.debuffAttachment.type];
            toolMoveFunctions.paint(ctx, createdObject.debuffAttachment, modPaintPos, ability, game);
        }
    }
    if (currentStage < 1) {
        for (let stageObjects of createdObject.nextStage) {
            paintToolObjectsRecusiveForCharging(ctx, abilityOwner, currentStage + 1, stageObjects, ability, modPaintPos, chargeFactor, spell, game);
        }
    }
}
    ...
    if (abilitySm.mode === "spellmake") {
        ...
        if (tool && tool.workInProgress) {
            if (tool.subType == "default") {
                ...
                const toolMoveFunctions = SPELLMAKER_DEBUFF_TOOLS_FUNCTIONS[tool.type];
                toolMoveFunctions.paint(ctx, tool.workInProgress, ownerPaintPos, abilitySm, game);
            }
        }
    } else {
        ...
        if (abilitySm.spelltypeChargeStart !== undefined) {
            ...
            for (let createdObject of currentSpell.createdObjects) {
                paintToolObjectsRecusiveForCharging(ctx, abilityOwner, 0, createdObject, abilitySm, paintPos, chargeFactor, currentSpell, game);
            }
        }
    }
}
                    ...
                    if (toolFunctions.onKeyUp) {
                        if (abilitySm.attachToIndex != undefined) {
                            ...
                            if (result) {
                                ...
                                currentObject.debuffAttachment = result;
                                abilitySpellmakerCalculateManaCostWithLevelFactor(abilitySm, currentSpell);
                            }
                        }
                    }
